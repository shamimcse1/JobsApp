...
public class JobListByCatActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        initHeader();
        viewBinding.toolbar.tvName.setText(categoryName);
        viewBinding.toolbar.fabBack.setOnClickListener(view -> onBackPressed());
    }

    private void onRequest() {
        if (NetworkUtils.isConnected(JobListByCatActivity.this)) {
            if (!isLoadMore) {
                mAdapter.onLoading();
            }
            getJobList();
    ...
    private void getJobList() {
        ...
        Call<JobListCallback> callback = apiInterface.getJobListByCat(API.toBase64(jsObj.toString()), pageIndex);
    ...
    private void initHeader() {
        if (myApplication.isLogin()) {
            GlideApp.with(JobListByCatActivity.this).load(myApplication.getLoginInfo().getUserImage()).placeholder(R.drawable.dummy_user).error(R.drawable.dummy_user).into(viewBinding.toolbar.includeImage.ivUserImage);
            viewBinding.toolbar.includeImage.ivUserStatus.setImageResource(R.drawable.online);
        }

        viewBinding.toolbar.includeImage.ivUserImage.setOnClickListener(view -> new ProfilePopUp(JobListByCatActivity.this, view));
    }
