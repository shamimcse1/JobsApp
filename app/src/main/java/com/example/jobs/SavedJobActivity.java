...
import com.example.adapter.SavedJobAdapter;
...
public class SavedJobActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mAdapter = new SavedJobAdapter(this);
        ...
        initHeader();
        viewBinding.toolbar.tvName.setText(getString(R.string.saved_job));
        viewBinding.toolbar.fabBack.setOnClickListener(view -> onBackPressed());

    }

    private void onRequest() {
        if (NetworkUtils.isConnected(SavedJobActivity.this)) {
            if (!isLoadMore) {
                mAdapter.onLoading();
            }
            getSavedJobList();
    ...
    private void getSavedJobList() {
        ...
        Call<JobListCallback> callback = apiInterface.getSavedJobList(API.toBase64(jsObj.toString()), pageIndex);
    ...
    private void initHeader() {
        if (myApplication.isLogin()) {
            GlideApp.with(SavedJobActivity.this).load(myApplication.getLoginInfo().getUserImage()).placeholder(R.drawable.dummy_user).error(R.drawable.dummy_user).into(viewBinding.toolbar.includeImage.ivUserImage);
            viewBinding.toolbar.includeImage.ivUserStatus.setImageResource(R.drawable.online);
        }

        viewBinding.toolbar.includeImage.ivUserImage.setOnClickListener(view -> new ProfilePopUp(SavedJobActivity.this, view));
    }
