...
import com.example.adapter.RecentJobAdapter;
...
public class RecentJobActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mAdapter = new RecentJobAdapter(this);
        ...
        initHeader();
        viewBinding.toolbar.tvName.setText(getString(R.string.recent_job));
        viewBinding.toolbar.fabBack.setOnClickListener(view -> onBackPressed());
    }

    private void onRequest() {
        if (NetworkUtils.isConnected(RecentJobActivity.this)) {
            if (!isLoadMore) {
                mAdapter.onLoading();
            }
            getJobList();
    ...
    private void getJobList() {
        ...
        Call<JobListCallback> callback = apiInterface.getRecentJobList(API.toBase64(jsObj.toString()), pageIndex);
    ...
    private void initHeader() {
        if (myApplication.isLogin()) {
            GlideApp.with(RecentJobActivity.this).load(myApplication.getLoginInfo().getUserImage()).placeholder(R.drawable.dummy_user).error(R.drawable.dummy_user).into(viewBinding.toolbar.includeImage.ivUserImage);
            viewBinding.toolbar.includeImage.ivUserStatus.setImageResource(R.drawable.online);
        }

        viewBinding.toolbar.includeImage.ivUserImage.setOnClickListener(view -> new ProfilePopUp(RecentJobActivity.this, view));
    }
