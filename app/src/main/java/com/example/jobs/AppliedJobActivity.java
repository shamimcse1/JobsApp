...
public class AppliedJobActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mAdapter = new AppliedJobAdapter(this);
        ...
        initHeader();
        viewBinding.toolbar.tvName.setText(getString(R.string.applied_job));
        viewBinding.toolbar.fabBack.setOnClickListener(view -> onBackPressed());
    }

    private void onRequest() {
        if (NetworkUtils.isConnected(AppliedJobActivity.this)) {
            if (!isLoadMore) {
                mAdapter.onLoading();
            }
            getSavedJobList();
    ...
    private void getSavedJobList() {
        ...
        Call<JobListCallback> callback = apiInterface.getAppliedJobList(API.toBase64(jsObj.toString()), pageIndex);
    ...
    private void initHeader() {
        if (myApplication.isLogin()) {
            GlideApp.with(AppliedJobActivity.this).load(myApplication.getLoginInfo().getUserImage()).placeholder(R.drawable.dummy_user).error(R.drawable.dummy_user).into(viewBinding.toolbar.includeImage.ivUserImage);
            viewBinding.toolbar.includeImage.ivUserStatus.setImageResource(R.drawable.online);
        }

        viewBinding.toolbar.includeImage.ivUserImage.setOnClickListener(view -> new ProfilePopUp(AppliedJobActivity.this, view));
    }
